...
import { ErrorHandler, AppError, ErrorCategory } from '../../src/utils/error-handler';
...
describe('ErrorHandler Utilities', () => {
  describe('Error Creation Helpers', () => {
    it('should create validation error with correct properties', () => {
      const error = ErrorHandler.validationError('Invalid input', 'email', 'bad@email');
    ...
    it('should create network error with correct properties', () => {
      const error = ErrorHandler.networkError('Connection failed', 'https://api.example.com');
  ...
  describe('withTimeout', () => {
    ...
    it('should use default timeout message when not provided', async () => {
      ...
      await expect(
        ErrorHandler.withTimeout(mockFn, 50)
  ...
  describe('Error Classification', () => {
    ...
    it('should classify error categories correctly', () => {
      expect(ErrorHandler.networkError('test').category).toBe(ErrorCategory.NETWORK);
      expect(ErrorHandler.databaseError('test').category).toBe(ErrorCategory.DATABASE);
      expect(ErrorHandler.validationError('test').category).toBe(ErrorCategory.VALIDATION);
      expect(ErrorHandler.authenticationError().category).toBe(ErrorCategory.AUTHENTICATION);
      expect(ErrorHandler.authorizationError().category).toBe(ErrorCategory.AUTHORIZATION);
    });
  });
